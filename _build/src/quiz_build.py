...
by=lambda *names:[c for c in cards.CLUBS if c['name'] in names]
CARDS={'royal':by('わかばFC','青空サッカークラブ'),'personal':by('世田谷ジュニアテニス','狛江キッズ空手'),'outdoor':by('杉並アスレチッククラブ','多摩川ジュニアSC'),'minor':by('さくらミニバスケットボールクラブ','港北ジュニアバスケ')}
CARDS_HTML={k:''.join(cards.card(**c) for c in v[:2]) for k,v in CARDS.items()}
js=r'''
(function(){
...
function render(){
 ...
 if(q.type==='area'){s+='<div class="cq-sel"><select><option>東京都</option></select><select><option>世田谷区</option></select></div><a class="cq-geo" href="#">◎ 現在地から探す</a>'}
 ...
 var nx=box.querySelector('.cq-next');if(nx)nx.addEventListener('click',function(){if(q.type==='area')ans.area=['世田谷区'];cur++;render()});
...
function result(){
 ...
 var cond=['世田谷区',age,day].concat((ans.cond||[]).map(function(i){return label('cond',i)}));if(r.tag.mood)cond.push(r.tag.mood);
 ...
 s+='<h3>まず見てほしい種目</h3><div class="sp">'+T.sports.map(function(x){return '<a href="search-preview.html"><b>'+h(x[0])+'</b><small>'+h(x[1])+'</small><i>世田谷区の'+h(x[0])+'を見る ›</i></a>'}).join('')+'</div>';
 s+='<h3>世田谷区で、いま募集しているクラブ</h3><div class="cards">'+(CARDS[r.type]||'')+'</div>';
 s+='<div class="save"><div class="t">この結果を、マイページに保存しますか。</div><p>保存すると、この条件に合う新しいクラブが載ったとき・体験の募集が始まったときにお知らせします。あとから条件も変えられます。</p><div class="row"><button class="b1 cq-save">保存する（無料）</button><a class="b2" href="search-preview.html">一覧で見る</a></div></div>';
 s+='<div class="alt">はじめてのクラブ選びなら：<a href="article-preview.html">「うちの子に合うクラブ」の見つけ方・5つの視点</a>／<a href="'+T.href+'">'+h(T.name)+'の種目をぜんぶ見る</a></div></div>';
 box.innerHTML=s;box.scrollTop=0;
 box.querySelector('.cq-redo').addEventListener('click',function(e){e.preventDefault();cur=0;render()});
 box.querySelector('.cq-save').addEventListener('click',function(){try{localStorage.setItem('chibispo_quiz',JSON.stringify({type:r.type,typeName:T.name,cond:cond,sports:T.sports.map(function(x){return x[0]}),at:new Date().toISOString()}))}catch(e){}location.href='mypage-preview.html?saved=1'});
}
...
function demo(){open();ans={area:['世田谷区'],age:[1],team:[0],place:[0],like:[0,1],mood:[0],day:[1],cond:[3]};cur=Q.length;render()}
...
'''
js=js.replace('__CARDS__',json.dumps(CARDS_HTML,ensure_ascii=False))
open(D+'preview-quiz.js','w',encoding='utf-8').write(js)
open(D+'preview-quiz.css','w',encoding='utf-8').write(open(P+'quiz.css',encoding='utf-8').read())
print('quiz files written', len(js)//1024,'KB')
