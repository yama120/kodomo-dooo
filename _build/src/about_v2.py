...
.abt-open .sc{position:absolute;left:50%;bottom:22px;z-index:2;transform:translateX(-50%);font-family:'Anton',sans-serif;font-size:10px;letter-spacing:.3em;color:#fff;opacity:.8;text-shadow:0 1px 8px rgba(0,0,0,.7)}
...
.abt-tri .in{max-width:1000px;margin:0 auto;display:grid;grid-template-columns:minmax(0,1fr);gap:40px;align-items:center}
.abt-tri .k{font-family:'Anton',sans-serif;font-size:12px;letter-spacing:.3em;color:var(--accent);margin-bottom:14px}
.abt-tri h2{font-family:var(--m);font-weight:600;font-size:clamp(22px,3.4vw,38px);letter-spacing:.05em;line-height:1.5;margin:0 0 20px}
.abt-tri h2 span{display:block;white-space:nowrap}
.abt-tri p{font-size:14px;font-weight:700;line-height:2.1;margin:0 0 12px}
.abt-dg{position:relative;width:min(100%,380px);max-width:100%;aspect-ratio:1;margin:0 auto;justify-self:center;min-width:0}
...
@media(min-width:760px){
  .abt-v{grid-template-columns:minmax(0,1fr) minmax(0,1fr);border-top:1px solid var(--ink);border-left:1px solid var(--ink)}
  .abt-v>div{padding:44px 40px 48px;border-right:1px solid var(--ink)}
  .abt-tri .in{grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:60px}
  .abt-cta{grid-template-columns:repeat(3,minmax(0,1fr))}
}
'''
body='''
<div class="abt">
...
<section class="abt-val">
  <div class="wrap">
    ...
    <div class="abt-v">
      ...
      <div class="rv"><div class="en">SUBSTANCE</div><div class="no">02</div><h3>値段ではなく、本質を。</h3><p>月謝は目安として載せます。でも、クラブの価値は金額では決まりません。コーチの声のかけ方、続けている子の顔ぶれ、練習の空気。<b>その子が続けられるかどうか</b>を決めるものを、いちばん大きく出します。</p></div>
...
<section class="abt-tri">
  <div class="in">
    <div class="rv">
      ...
      <h2><span>まんなかに、</span><span>子どもがいる。</span></h2>
...
<section class="abt-let">
  <div class="in rv">
    ...
    <h2>運営者より</h2>
    <p>チビスポは、からだの動きに毎日向き合っている理学療法士が、子どもの成長を手助けしたくて始めた、小さな場所です。</p>
    <p>専門知識は、押しつけるためのものではありません。保護者が安心して一歩を踏み出せるように。現場感とエビデンスの両方を、やさしい言葉でお届けしていきます。</p>
    <p>「探せない」「伝わらない」「分からない」を、このサービスで解決していきます。</p>
    <p>クラブの方へ。あなたの練習の空気は、文章より雄弁です。まずは一枚の写真から、載せてみてください。</p>
    <div class="sg">チビスポ 運営者<small>理学療法士／山田</small></div>
  </div>
</section>
