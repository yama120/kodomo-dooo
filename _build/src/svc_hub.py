...
HUB_NUM_CLUBS='147'   # 掲載クラブ数の初期値（2026-09-26時点の approved 実数）。表示時に hub_js がDBの実数で上書きする
HUB_NUM_PREF='25'     # 都道府県の初期値（approved の pref の種類数）。同じく表示時にDBで上書き
...
<section class="hub-hero" id="top">
  ...
  <div class="in">
    <div class="cp">
      ...
      <div class="stats"><div><b data-cnt="%(clubs)s" data-live="clubs">0</b><span>CLUBS</span></div><div><b data-cnt="%(pref)s" data-live="pref">0</b><span>PREFECTURES</span></div><div><b>0<small>円</small></b><span>LISTING FEE</span></div></div>
    </div>
...
hub_js='''<script>
(function(){
 /* 掲載クラブ数・都道府県数はDBの公開中（approved）から数える。取れなければ初期値のまま */
 var LIVE=fetch('https://emkpkomrgknzrmxqbrvx.supabase.co/rest/v1/teams?select=pref&status=eq.approved',{headers:{apikey:'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVta3Brb21yZ2tuenJteHFicnZ4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY5Nzg0MTYsImV4cCI6MjA5MjU1NDQxNn0.YmtVc_0le-EDjGzv1PJHet0ShnhfFZLIYT587FzcJHQ',Authorization:'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImVta3Brb21yZ2tuenJteHFicnZ4Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzY5Nzg0MTYsImV4cCI6MjA5MjU1NDQxNn0.YmtVc_0le-EDjGzv1PJHet0ShnhfFZLIYT587FzcJHQ'}})
  .then(function(r){return r.ok?r.json():null}).then(function(rows){
   if(!rows||!rows.length)return;
   var pf={};rows.forEach(function(t){if(t.pref)pf[t.pref]=1});
   var v={clubs:rows.length,pref:Object.keys(pf).length};
   document.querySelectorAll('[data-live]').forEach(function(b){var n=v[b.dataset.live];if(!n)return;b.dataset.cnt=n;if(b.dataset.done)b.textContent=n});
  }).catch(function(){});
 ...
 if(/noanim=1/.test(location.search)){
   ...
   document.querySelectorAll('[data-cnt]').forEach(function(b){b.textContent=b.dataset.cnt;b.dataset.done=1});
   return;
 }
 ...
 document.querySelectorAll('[data-cnt]').forEach(function(b){var t0=null;function f(t){if(!t0)t0=t;var p=Math.min(1,(t-t0)/900);b.textContent=Math.round(+b.dataset.cnt*(1-Math.pow(1-p,3)));if(p<1)requestAnimationFrame(f);else b.dataset.done=1}setTimeout(function(){requestAnimationFrame(f)},400)});
