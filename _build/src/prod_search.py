...
/* 雰囲気タグ：4行までにして、続きは「もっと見る」で開く */
#sr-mood-list.cl{overflow:hidden;position:relative}
...
<div class="wrap">
  ...
  <aside class="fside mag" id="fside">
    ...
    <div id="sr-mood-wrap" hidden><div class="lb">雰囲気</div><div id="sr-mood-list"></div><button type="button" id="sr-mood-more" aria-expanded="false" hidden>もっと見る</button></div>
...
(function(){
  ...
  teamsQuery().then(function(res){
    ...
    (function(){
      ...
      wrap.hidden=false;
      /* 4行までにして、続きは「もっと見る」で開く（行の高さは実測する） */
      var more=document.getElementById('sr-mood-more'), open=false, maxH=0;
      function measure(){
        list.classList.remove('cl'); list.style.maxHeight='';
        var tops=[], kids=list.children, i, t;
        for(i=0;i<kids.length;i++){ t=kids[i].offsetTop; if(tops.indexOf(t)<0) tops.push(t); }
        tops.sort(function(a,b){ return a-b; });
        if(tops.length<=4 || !more){ if(more) more.hidden=true; return; }
        maxH = tops[3]-tops[0] + (kids[0].offsetHeight||30);
        var n=0; for(i=0;i<kids.length;i++){ if(kids[i].offsetTop>tops[3]) n++; }
        more.hidden=false; more.textContent = open ? '閉じる' : 'もっと見る（あと'+n+'）';
        if(!open){ list.style.maxHeight=maxH+'px'; list.classList.add('cl'); }
      }
      if(more){
        more.addEventListener('click',function(){
          open=!open;
          if(open){ list.classList.remove('cl'); list.style.maxHeight=''; more.textContent='閉じる'; }
          else { list.style.maxHeight=maxH+'px'; list.classList.add('cl'); }
          more.setAttribute('aria-expanded', open?'true':'false');
          if(!open) measure();
        });
      }
      measure();
      /* 書体が当たると1行に入る数が変わるので、読み込み後にもう一度測る */
      try{ if(document.fonts && document.fonts.ready) document.fonts.ready.then(measure); }catch(e){}
      var rt; addEventListener('resize',function(){ clearTimeout(rt); rt=setTimeout(measure,180); });
      /* URL で指定された雰囲気が畳んだ先にあるときは開いておく */
      if(_p.get('mood') && more && !more.hidden){ more.click(); }
    })();
